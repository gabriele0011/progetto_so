...
file* replacement_algorithm(file* cache, size_t dim_f, char* dirname);
...
int cache_appendToFile(file** cache, char* f_name, byte* f_data, size_t dim_f, char* dirname)
{
	...
	if(cache_capacity_control(dim_f) == -1){
		...
		if((temp = replacement_algorithm(*cache, dim_f, dirname)) != NULL){
			...
			if(cache_removeFile(cache, node->f_name) == -1){
...
int main(){
	...
	cache_writeFile(&cache, f_name1, data1, 4, dirname);
	//print_queue(cache);

	char f_name2[6] = {'f', 'i', 'l', 'e', '2', '\0'};
	byte data2[8] = {'c', 'o', 'm', 'e', 's', 't', 'a', '\0'};
	cache_writeFile(&cache, f_name2, data2, 8, dirname);
	//print_queue(cache);
	...
	cache_appendToFile(&cache, f_name3, datax, 4, NULL);
...
/*	
	char f_name2[6] = {'f', 'i', 'l', 'e', '2', '\0'};
	byte data2[4] = {'c', 'i', 'a', '\0'};
	cache_writeFile(&cache, f_name2, data2, 4, dirname);
...
*/
	...
	//cache_removeFile(&cache, f_name);
	//print_queue(cache);
	
	//cache_removeFile(&cache, f_name1);
	//print_queue(cache);
	/*
	cache_removeFile(&cache, f_name3);
	print_queue(cache);

	cache_removeFile(&cache, f_name4);
	*/
	print_queue(cache);
	return 0;
